...
st.info(
    "💡 **ニックネームでも診断できます。**\n\n"
    "本名を入力された方は、**姓名判断も含めた詳細レポート**になります。"
)

name_mode = st.radio(
    "名前の入力方法",
    options=["ニックネームで入力（姓名判断なし）", "本名で入力（姓名判断あり・詳細版）"],
    index=0,
    horizontal=False,
)
...
st.divider()
st.subheader("🛡️ Step 2：信頼される男タイプ診断（20問）")
st.caption("各設問について、当てはまる度合いをお選びください。所要時間 約3〜5分。")
st.info(
    "💡 5段階で素直に答えてください。深く考えず、最初の直感でOKです。\n\n"
    "5タイプ（論理派／責任型／安定型／社交型／共感型）から、**メインタイプ + 隠れタイプ**が判定されます。"
)

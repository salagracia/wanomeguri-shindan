...
st.markdown('<h1 class="main-header">🛡️ 45歳からの 信頼される男 タイプ診断</h1>', unsafe_allow_html=True)
st.markdown('<p class="sub-header">仕事で頭打ちを感じている男性のための、行動パターン分析</p>',
            unsafe_allow_html=True)

st.markdown("""
仕事で結果を出してきたのに、なぜか家ではうまく噛み合わない瞬間がある。
それは、**能力でも年齢でもなく、「強みの使い場所」が少しズレているだけ**かもしれません。

このアプリでは、**生年月日・名前・20問の質問回答**をもとに、あなたの「**信頼タイプ**」を読み解きます。

🛡️ **行動診断**：仕事軸・家庭軸・価値観軸から、5タイプを判定（メイン＋隠れタイプ）
📊 **データ補強**：生年月日・名前から、判定精度を高める
✍️ **自由記述（任意）**：書ける範囲で・飛ばしてもOK

**所要時間：約5〜8分**　→ 結果はその場で表示・**個人レポートPDFをダウンロード可能**
""")
...
st.info(
    "💡 **ニックネームでも診断できます。**\n\n"
    "本名を入力された方は、お名前の構造も加味した**詳細レポート**になります。"
)
...
# ========== Step 2: 信頼される男タイプ診断（20問・5段階Likert） ==========
st.divider()
st.subheader("🛡️ Step 2：行動パターンの設問（20問）")
st.caption("各設問について、当てはまる度合いをお選びください。所要時間 約3〜5分。")
st.info(
    "💡 5段階で素直に答えてください。深く考えず、最初の直感でOKです。\n\n"
    "回答結果から、5タイプ（論理派／責任型／安定型／社交型／共感型）のうち、**メインタイプと隠れタイプ**を読み解きます。"
)

# v4.0: 質問順序をセッションごとに1度だけランダム化
...
# v4.0: メアド入力廃止。基本情報＋20問以上で診断OK。結果は画面表示＋PDFダウンロード。
...
if st.button("🛡️ あなたの「信頼される男タイプ」を診断する", disabled=not input_valid):
    with st.spinner("回答パターンとプロファイルを分析中..."):
        user_input = {
            ...
            # v4.0: 本名なら姓名判断章を出力、ニックネームならスキップ
        ...
        try:
            ...
            # 詳細プロファイル要約（PDFレポートに同データが収録）
            with st.expander("📊 詳細プロファイル（補足データ）"):
                st.caption("生年月日・名前から導かれる、補強データの要約です。詳細はPDFレポート参照。")
                col_a, col_b = st.columns(2)
                with col_a:
                    st.markdown(f"**生まれ持った傾向①：** {result['western_astrology']['sun']['name']}")
                    st.markdown(f"**生まれ持った傾向②：** {result['western_astrology']['moon']['name']}")
                    st.markdown(f"**人生のテーマ数：** {result['numerology']['life_path']['number']}")
                    st.markdown(f"**行動の主軸：** {result['shusei']['name']}")
                with col_b:
                    st.markdown(f"**個性プロファイル：** {result['doubutsu']['name_60']}")
                    st.markdown(f"**リーダーシップ型：** {result['teiou']['name']}")
                    st.markdown(f"**気質：** {result['kyusei']['honmei']['name']}")
                    tc_periods = result.get('tenchusatsu_years', {})
                    next_p = tc_periods.get('next_period')
                    if next_p:
                        st.markdown(f"**次の立て直し期：** {next_p[0]}〜{next_p[1]}年")
            ...
            st.markdown(
                "20問のタイプ判定＋詳細プロファイル＋自由記述から、**あなただけの個人レポート**として全章書き出しました。"
